...
class AgentCheck(object):
    ...
    def load_configuration_models(self):
        ...
        if self.__shared_config is None:
            shared_config = self.load_configuration_model(package_path, 'SharedConfig', self._get_shared_config())
            if shared_config is not None:
                self.__shared_config = shared_config

        if self.__config is None:
            instance_config = self.load_configuration_model(package_path, 'InstanceConfig', self._get_instance_config())
